module game {
	export class GameZhuaPaiQiUI extends egret.Sprite {
		...
		private init(): void {
			...
			this.parItem.addChild(item);
			item.x = 0;
			item.y = Math.floor(26 / 9) * itemWidth + Math.floor(26 / 9) * 25;
			item.addEventListener(egret.TouchEvent.TOUCH_TAP, this.onClick, this);
			item.touchEnabled = true;
			let txt: egret.TextField = new egret.TextField();
			txt.text = "红中";
			...
			item2.y = Math.floor(26 / 9) * itemWidth + Math.floor(26 / 9) * 25;
			...
			item3.y = Math.floor(26 / 9) * itemWidth + Math.floor(26 / 9) * 25;
			...
			item12.x = 210;
			item12.y = Math.floor(26 / 9) * 50 + Math.floor(26 / 9) * 25 + 150;
			...
			item13.y = Math.floor(26 / 9) * 50 + Math.floor(26 / 9) * 25 + 150;
		...
		private onClick(e: egret.TouchEvent): void {
			...
			room.RoomWebSocket.instance().roomSender.REQ_MAGICTILES(Number(btn.name));
		}
	}
}